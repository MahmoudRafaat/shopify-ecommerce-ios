...
struct SettingsProfileHeader: View {
    ...
    var body: some View {
        HStack(spacing: 16) {
            Circle()
                .fill(isLoggedIn ? AppColor.brandPrimary.opacity(0.2) : AppColor.textSecondary.opacity(0.2))
                .frame(width: 60, height: 60)
                .overlay {
                    Image(systemName: isLoggedIn ? "person.fill" : "person.slash.fill")
                        .font(.title2)
                        .foregroundStyle(isLoggedIn ? AppColor.brandPrimary : AppColor.textSecondary)
                }
